    ...
    >
      ...
      <div className="mx-auto flex min-h-full w-full max-w-xl items-start justify-center p-4 sm:items-center sm:p-6">
        <div className="surface-elevated w-full rounded-xl bg-[color:var(--color-surface-muted)]/78 p-4 sm:p-5" role="status" aria-live="polite">
          ...
          <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-[color:var(--color-border)]">
            <div
              className="h-full rounded-full bg-[image:var(--gradient-primary)] transition-[width] duration-500 ease-out"
            ...
            {generationSteps.map((step, index) => {
              ...
              return (
                <li
                  key={step.label}
                  className={cn(
                    'surface-elevated flex items-start justify-between gap-4 rounded-xl px-4 py-3 text-sm transition-colors',
